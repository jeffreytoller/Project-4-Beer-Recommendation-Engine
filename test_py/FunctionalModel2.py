...
def prepare_data():
    ...
    count_df = df.groupby('beer_beerid').count()
    cols = count_df.columns
    count_df['review_count'] = count_df.brewery_id
    count_df.drop(columns=cols,inplace=True)
    ...
    beers = beers.merge(right=count_df,how='left',on='beer_beerid')
...
def get_recommendations(merge_df,user_df,beers_df):
    ...
    pred_df['weighted_rating'] =  pred_df.predicted_rating*(pred_df.review_count**(1/20))
    pred_df = pred_df.sort_values('weighted_rating',ascending=False)
    return(pred_df)

# %%
def get_all_ratings(preceding_tr, user_name,page_index = 0):
    user_url = 'https://www.beeradvocate.com/user/beers/?start='+ str(page_index) + '&&ba=' + user_name + '&order=dateD&view=R#lists'
    res = extract_ratings(preceding_tr,user_url)
    if not res.empty:
        print(page_index)
        page_index+=50
        return(pd.concat([res,get_all_ratings(preceding_tr,user_name,page_index)],axis=0,ignore_index=True))
    else:
        return(pd.DataFrame())

# %%
def run_recommender(username,beers_df,merge_df):
    ratings = get_all_ratings(3,username)
